...
namespace CanhCam.Business
{
    public class News : IIndexableContent
    {
        ...
        public static List<News> GetPageBySearch(
            ...
            int pageSize)
        {
            return LoadListFromReader(DBNews.GetPageBySearch(siteId, listZoneId, newsType, isPublished, listStateId, languageId, position, showOption, startDateFrom, startDateTo, endDateFrom, endDateTo, userGuid,authorId,newstypeId, keyword, pageNumber, pageSize));
        }
        ...
        protected static List<News> LoadListFromReader(IDataReader reader, bool loadUser = true, bool loadImage = true)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (reader["ShareCount"] != DBNull.Value)
                    {
                        item.ShareCount = int.Parse(reader["ShareCount"].ToString());
                    }
                    if (reader["LikeCount"] != DBNull.Value)
                    {
                        item.LikeCount = int.Parse(reader["LikeCount"].ToString());
                    }
                    if (reader["AuthorID"] != DBNull.Value)
                    {
                        item.AuthorID = int.Parse(reader["AuthorID"].ToString());
                    }
                    if (reader["NewType"] != DBNull.Value)
                    {
                        item.NewstypeID = int.Parse(reader["NewType"].ToString());
                    }
